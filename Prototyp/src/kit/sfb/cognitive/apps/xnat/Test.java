package kit.sfb.cognitive.apps.xnat;

import java.io.File;
import java.io.IOException;
import java.net.URI;
import java.net.URISyntaxException;

public class Test {

	public static void main(String[] args) throws URISyntaxException, IOException {
		// TODO Auto-generated method stub

		// System.out.println(Uploader.uploadToProject("TP",
		// "C:/Users/phiL/Desktop/test_upload3.xml"));
		// System.out.println(Uploader.uploadToSubject("TP", "sfb125_S00002",
		// "C:/Users/phiL/Desktop/test_upload3.xml"));
		// System.out.println(Uploader.uploadToExperiment("sfb125_E00004",
		// "C:/Users/phiL/Desktop/test_upload3.xml"));
		URI uri = new URI("https://xnat.sfb125.de/data/projects/TP/resources/2/files/test_upload2.xml");
		File test = Downloader.downloadFile(uri);

	}

}
